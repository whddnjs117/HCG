/**
 * # 신규 버전 패치 시 참고사항
 *  - 신규 버전이 배포되었을 경우 소스를 비교하여 변경된 부분을 반영합니다.
 *  - 단, hunel에 맞춰 수정된 부분이 있습니다. 해당 스크립트를 확인한 후 수정합니다.
 *  - 수정된 소스는 아래와 같이 표기하였습니다.(주석)
 *  
 *     @ hunel custom [add:추가소스, modify:변경소스]
 *     ...
 *     (수정한 스크립트)
 *     ...
 *     hunel custom @
...
/* @ hunel custom [add] */ 
    div_str += "<div class='tableBox noscroll'>";
/*   hunel custom @     */
 ...
 */
/* @ hunel custom [modify] */ 
function FormQueryString(form, checkRequired, jsonObj) {
/*   hunel custom @        */
...
/* @ hunel custom [add] */ 
    //페이지 보안검사를 위해
    name[j] = "S_PGM_OPEN_TIME";
    value[j] = S_PGM_OPEN_TIME;
    j++;
    name[j] = "S_ENC_OTP_KEY";
    value[j] = S_ENC_OTP_KEY;
    j++;
    name[j] = "S_CSRF_SALT";
    value[j] = S_CSRF_SALT;
    j++;
...
/* @ hunel custom [modify] */
    ...
    for (i = 0; i < j; i++) 
    {
        if (name[i] != '' && name[i] != "__viewState")
        { 
          if(name[i] == "S_DSCLASS" || name[i] == "S_DSMETHOD" || name[i] == "S_ENC_OTP_KEY")
          {
...
/* @ hunel custom [add] */ 
    //jsonObj를 QueryString에 추가한다.
    //ibsheetinfo.js 패치시 적용해야할 부분
    if( jsonObj != null)
    {
      for(var i in jsonObj)
      {
        if( ! isinarr(i, name))
        {
          plain_text += encodeURIComponent(i) + "=" + encodeURIComponent((jsonObj[i] == undefined ? "" : jsonObj[i])) + "&";
        }
      }
    } 
/*   hunel custom @     */
...
/* @ hunel custom [add] */ 
    $("#__viewState").val(toJsonString(toQueryParams(plain_text)));
    plain_text += "&__viewState="+$("#__viewState").val();
/*   hunel custom @     */
 ...
 */
/* @ hunel custom [modify] */ 
function FormQueryStringEnc(form, checkRequired, jsonObj) {
/*   hunel custom @        */
    if (typeof form != "object" || form.tagName != "FORM") {
